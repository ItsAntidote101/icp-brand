...
// ─── Email 14: Auth OTP code ──────────────────────────────────────────────────
// Sent from the Supabase Auth "Send Email" hook (src/app/api/auth/hooks/send-email)
// so login codes match the rest of the brand instead of Supabase's default
// auth template. Covers both customer and media buyer sign-in.

const OTP_COPY: Record<string, { heading: string; sub: string; subject: string }> = {
  signup: {
    heading: 'Confirm your email',
    sub: 'Welcome to Ideal ICP. Use this code to confirm your email and sign in.',
    subject: 'Your Ideal ICP confirmation code',
  },
  magiclink: {
    heading: 'Your login code',
    sub: 'Use this code to sign in to Ideal ICP.',
    subject: 'Your Ideal ICP login code',
  },
  email_change: {
    heading: 'Confirm your new email',
    sub: 'Use this code to confirm your new email address.',
  ...
  reauthentication: {
    ...
    sub: 'Use this code to confirm it is really you.',
...
export async function sendOtpEmail({
  to, token, actionType,
}: { to: string; token: string; actionType: string }) {
  const { heading: h, sub: s, subject } = OTP_COPY[actionType] ?? {
    heading: 'Your verification code',
    sub: 'Use this code to continue.',
    subject: 'Your Ideal ICP verification code',
  }
...
<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 8px;"><tr>
  <td style="background-color:${CardBg};border:1.5px solid ${Border};border-radius:6px;padding:20px 32px;">
    <p style="margin:0;color:${Dark};font-size:34px;font-weight:800;letter-spacing:8px;font-family:${font};text-align:center;">${escapeHtml(token)}</p>
  </td>
</tr></table>
<p style="margin:16px 0 0;color:${Muted};font-size:13px;font-family:${font};">This code expires shortly. If you did not request this, you can safely ignore this email.</p>`
  ...
  if (error) console.error('[email] otp error:', JSON.stringify(error))
  else console.log('[email] otp sent id:', data?.id, 'to:', to, 'type:', actionType)
  return { data, error }
}
