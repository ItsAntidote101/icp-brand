...
function AccountTab({ user, currency, score, reportCount, reports, onSignOut, onCancelled, onUserUpdate, showToast, onUpgrade, buyer }: {
  ...
  const [refShareUrl,      setRefShareUrl]       = useState<string | null>(null)
  ...
  useEffect(() => {
    fetch('/api/referral/me')
      .then(r => r.ok ? r.json() : null)
      .then((d: { code: string; shareUrl: string; referralCount: number; bonusDiagnoses: number } | null) => {
        if (!d) return
        setRefCode(d.code)
        setRefShareUrl(d.shareUrl)
        setRefCount(d.referralCount)
        setRefBonus(d.bonusDiagnoses)
      })
      .catch(() => {})
  }, [])
  ...
  function handleCopyRef() {
    ...
    navigator.clipboard.writeText(refShareUrl).then(() => {
      setRefCopied(true)
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card style={{ padding: '24px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <p style={{ fontFamily: fontB, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: Pmuted, margin: 0 }}>Refer a friend</p>
            {refBonus > 0 && (
              <span style={{ background: 'rgba(232,51,10,0.08)', color: '#e8330a', fontFamily: fontB, fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 4 }}>
                +{refBonus} bonus {refBonus === 1 ? 'diagnosis' : 'diagnoses'} earned
              </span>
            )}
          </div>
          <p style={{ fontFamily: fontB, fontSize: 13, color: Pmuted, lineHeight: 1.6, margin: '0 0 16px' }}>
            Share your link. When a friend completes their first ICP score, you both benefit — you earn a bonus diagnosis you can use any time.
          </p>

          {/* Link row */}
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            <div style={{ flex: 1, background: BgAlt, border: `1px solid ${Pborder}`, borderRadius: 8, padding: '9px 12px', overflow: 'hidden' }}>
              <p style={{ fontFamily: fontB, fontSize: 12, color: refShareUrl ? P : Pmuted, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {refShareUrl ?? 'Generating your link…'}
              </p>
            </div>
            <button onClick={handleCopyRef} disabled={!refShareUrl}
              style={{ flexShrink: 0, background: refCopied ? '#16a34a' : P, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 16px', fontFamily: fontB, fontSize: 12, fontWeight: 600, cursor: refShareUrl ? 'pointer' : 'default', transition: 'background 0.2s', whiteSpace: 'nowrap' }}>
              {refCopied ? 'Copied!' : 'Copy link'}
            </button>
          </div>

          {/* WhatsApp share */}
          <button onClick={handleWhatsAppRef} disabled={!refShareUrl}
            style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 16px', fontFamily: fontB, fontSize: 12, fontWeight: 600, cursor: refShareUrl ? 'pointer' : 'default', marginBottom: 14 }}>
          ...
          {/* Stats */}
          {(refCount > 0 || refBonus > 0) && (
            <div style={{ display: 'flex', gap: 24, paddingTop: 14, borderTop: `1px solid ${Pborder}` }}>
              <div>
                <p style={{ fontFamily: fontB, fontSize: 18, fontWeight: 700, color: P, margin: '0 0 2px' }}>{refCount}</p>
                <p style={{ fontFamily: fontB, fontSize: 11, color: Pmuted, margin: 0 }}>{refCount === 1 ? 'friend' : 'friends'} referred</p>
              </div>
              <div>
                <p style={{ fontFamily: fontB, fontSize: 18, fontWeight: 700, color: '#e8330a', margin: '0 0 2px' }}>{refBonus}</p>
                <p style={{ fontFamily: fontB, fontSize: 11, color: Pmuted, margin: 0 }}>bonus {refBonus === 1 ? 'diagnosis' : 'diagnoses'} earned</p>
              </div>
            </div>
          )}
          {refCount === 0 && (
            <p style={{ fontFamily: fontB, fontSize: 12, color: Pmuted, margin: 0 }}>
              No referrals yet. Share your link — every friend who completes their first diagnosis earns you a bonus re-run.
            </p>
          )}
        </Card>
